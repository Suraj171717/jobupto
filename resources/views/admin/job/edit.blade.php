...
<div class="fill-form" style="background-color: #ffd6c3;">

	<div class="hedi"><h3>Edit Job</h3></div>

	<div class="filla">
		@foreach($Jobs as $job)
		<form action='' id='jobadd' enctype="multipart/form-data" method="post">
            <input type="hidden" id="token" value="{{ csrf_token() }}" />
            <input type="hidden" id="idjob" name="id" value="{{ $job->id }}" />
		
			<div class="txtareadiv" >
				<div class="form-group" style="width:auto;" >
					<label class="col-md-4 control-label">Vacancies</label>
					<textarea id="Vacancies" class="form-control" name="job_vacancies" required >{{$job->job_vacancies}}</textarea><span class="red" style="color:red; position: absolute;" id="Vacancies-alert"></span>
				</div>		
			</div>		
			...
			<div class="form-group">
                ...
                <select id="Location" class="form-control" name="jobloc_id" required >
					<option value="{{$job->locationsid}}" >{{$job->locations}}</option>
					@foreach($locs as $loc)
						<option value="{{$loc->id}}">{{ucwords($loc->name)}}</option>
					@endforeach
				</select><span class="red" style="color:red; position: absolute;" id="Location-alert"></span>
			</div>
			...
			<div class="form-group">
                ...
                <select id="Category" class="form-control" name="jobcat_id" required >
					<option value="{{$job->categoriesid}}" >{{$job->categories}}</option>
					@foreach($cats as $cat)
						<option value="{{$cat->id}}">{{ucwords($cat->name)}}</option>
					@endforeach
				</select><span class="red" style="color:red; position: absolute;" id="Category-alert"></span>
			</div>
			...
			<div class="form-group">
				...
				<select id="Post" class="form-control" name="jobpost_id" required >
					<option value="{{$job->postsid}}" >{{$job->posts}}</option>
					@foreach($posts as $post)
						<option value="{{$post->id}}">{{ucwords($post->name)}}</option>
					@endforeach
				</select><span class="red" style="color:red; position: absolute;" id="Post-alert"></span>
			</div>
			...
			<div class="form-group">
                ...
                <select id="Qualification" class="form-control" name="jobqual_id" required >
					<option value="{{$job->qualificationsid}}" >{{$job->qualifications}}</option>
					@foreach($quals as $qual)
						<option value="{{$qual->id}}">{{ucwords($qual->name)}}</option>
					@endforeach
				</select><span class="red" style="color:red; position: absolute;" id="Qualification-alert"></span>
			</div>
			...
			<div class="form-group">
                ...
                <select id="Experience" class="form-control" name="jobexp_id" required >
					<option value="{{$job->experiencesid}}" >{{$job->experiences}}</option>
					@foreach($exps as $exp)
						<option value="{{$exp->id}}">{{ucwords($exp->name)}}</option>
					@endforeach	
				</select><span class="red" style="color:red; position: absolute;" id="Experience-alert"></span>
			</div>
			...
			<div class="form-group">
                ...
                <input type="text" id="Company" value="{{$job->job_company}}" class="form-control" name="job_company" required /><span class="red" style="color:red; position: absolute;" id="Company-alert"></span>
			</div>
			
			
			<div class="form-group" id="packwidth">
                <label class="col-md-4 control-label">Package</label>
                <input type="text" id="Package" value="{{$job->job_status}}" class="form-control" name="job_status" required /><span class="red" style="color:red; position: absolute;" id="Package-alert"></span>
			</div>
			
			
			<div class="form-group" id="lastdatewidth">
                <label class="col-md-4 control-label">Last_Date</label>
                <input type="text" id="Last_Date" value="{{$job->lastdate}}" class="form-control" name="lastdate" required /><span class="red" style="color:red; position: absolute;" id="Last_Date-alert"></span>
			</div>
			
			<div class="form-group">
                <label class="col-md-4 control-label">Final_Date</label>
                <input type="text" id="finaldate" value="{{$job->final_date}}" class="form-control" name="final_date" required />
			...
			<div class="form-group">
                ...
                <input type="file" name="advt" id="Advertisement" class="form-control" /><span class="red" style="color:red; position: absolute;" id="Advertisement-alert"></span>
			</div>
		
			
			<div class="form-group">
                <label class="col-md-4 control-label">Apply</label>
                <input type="text" id="Apply" value="{{$job->applynow}}" class="form-control" name="applynow" required /><span class="red" style="color:red; position: absolute;" id="Apply-alert"></span>
			</div>
